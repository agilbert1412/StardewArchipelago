...
namespace StardewArchipelago.Extensions
{
    public static class GameLocationExtensions
    {
        ...
        private static readonly Dictionary<WarpRequest, WarpRequest> ExtraWarps = new()
        {
            ...
            { new WarpRequest("Forest", 118, 36, FacingDirection.Left), new WarpRequest("Beach", 0, 6, FacingDirection.Right) },
            { new WarpRequest("Forest", 118, 36, FacingDirection.Left), new WarpRequest("BeachNightMarket", 0, 6, FacingDirection.Right) },
            ...
            foreach (var building in origin.buildings)
            {
                try
                {
                    ...
                    foreach (var warp in interior.warps)
                    {
                        ...
                        {
                            buildingWarps.Add(new Point(warp.TargetX, warp.TargetY), new Point(warp.X, warp.Y - 1));
                        }
                    }
                }
                catch (Exception ex)
